...
function Startup()
	...
	for k, v in ipairs(config.Restaurants) do
		plsr.Logger:Trace("Restaurant", string.format("Registering Restaurant ^3%s^7", v.Name))
		if v.Benches then
			for benchId, bench in pairs(v.Benches) do
				...
				if v?.IgnoreDuty then
					plsr.Crafting:RegisterBench(string.format("%s-%s", k, benchId), bench.label, bench.targeting, {
		...
		if v.Storage then
			for _, storage in pairs(v.Storage) do
				...
				plsr.Inventory.Poly:Create(storage)
			end
		end
